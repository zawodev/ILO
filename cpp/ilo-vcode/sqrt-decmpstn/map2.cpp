...
void update(int i, int val){
    int blockNumber = i / csize;
    c[blockNumber] += val - a[i];
    a[i] = val;
}
int getsqrdcmp(int l, int r){
    int score = 0;
    while (l<r and l%csize!=0 and l!=0){
        score += a[l];
        l++;
    }
    while (l <= r - csize){
